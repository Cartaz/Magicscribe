...
Button {
    ...
    readonly property bool insetState: selected || pressed
    ...
    background: Item {
        RectangularShadow {
            ...
            offset: control.hovered && !control.pressed ? Qt.vector2d(4.5, 4.5) : Qt.vector2d(3.5, 3.5)
            blur: control.hovered && !control.pressed ? 12 : 10
            radius: Theme.radiusSmall
            color: Theme.shadowDarkSoft
            opacity: control.insetState ? 0.0 : (control.enabled ? 1.0 : 0.35)
            cached: false
        }
        RectangularShadow {
            anchors.fill: buttonSurface
            offset: control.hovered && !control.pressed ? Qt.vector2d(-3.8, -3.8) : Qt.vector2d(-3, -3)
            blur: control.hovered && !control.pressed ? 10 : 8.5
            radius: Theme.radiusSmall
            color: Theme.shadowLightSoft
            opacity: control.insetState ? 0.0 : (control.enabled ? 1.0 : 0.3)
            cached: false
        }
        RectangularShadow {
            anchors.fill: buttonSurface
        ...
        Rectangle {
            ...
            radius: Theme.radiusSmall
            color: Theme.surface
            border.width: control.activeFocus && !control.insetState ? 1 : 0
            border.color: Theme.accent
            opacity: control.enabled ? 1.0 : 0.52
        }
        InsetSurface {
            anchors.fill: parent
            visible: control.insetState
            radius: Theme.radiusSmall
